...
module.exports = {
    id: 'Ticket_Nut_RemoveDC',
    description: 'Nút này cho phép người dùng có ADM xóa tất cả các kênh ticket có tên bắt đầu bằng "discord-".',
    async execute(interaction, client) {
    try {

            // Kiểm tra quyền của người dùng trước khi thực hiện thao tác
            if (interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                const ticketChannels = interaction.guild.channels.cache.filter(channel => /^discord-/i.test(channel.name));
                ...
                await interaction.reply({ content: `✅ Đã xóa tất cả các kênh ticket vé discord.`, ephemeral: true });
            } else {
