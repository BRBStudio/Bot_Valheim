const { SlashCommandBuilder, SlashCommandSubcommandBuilder, SlashCommandSubcommandGroupBuilder, ContextMenuCommandBuilder, EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder } = require('discord.js');
const fs = require('fs');
...
const fileEmojis = {
    ...
    'check_version_bot': `<a:diamond1:1318123134787911720>`,
    ...
    'wh': '<a:diamond1:1318123134787911720>',
    ...
    'promote_role': '<a:Admin:1319981885002354781>',
    ...
    'registeryt': '<a:Admin:1319981885002354781>',
    ...
    'check_role': '<a:Pubg:1320080665957367838>',
    'clear_user': '<a:Pubg:1320080665957367838>',
    'bot': '<a:Pubg:1320080665957367838>',
    'new': '<a:Pubg:1320080665957367838>',
    ...
    // 13. THỬ NGHIỆM (4)
    'âs': '<a:vote_star:1343454905351802930>',
};
...
// Hàm duyệt đệ quy thư mục để lấy tất cả file .js
function getCommandFiles(dir) {
    ...
    list.forEach(file => {
        ...
        if (stat && stat.isDirectory()) {
            results = results.concat(getCommandFiles(filePath)); // Duyệt đệ quy thư mục con
    ...
    async execute(interaction, client) {
        ...
        for (const folder of commandFolders) {
            ...
            for (const filePath of commandFiles) {
                const commandFile = require(path.resolve(filePath));
                if (commandFile && commandFile.data) {
                    let isSubcommand = commandFile.data instanceof SlashCommandSubcommandBuilder || commandFile.data instanceof SlashCommandBuilder; // SlashCommandBuilder
                    const isContextMenuCommand = commandFile.data instanceof ContextMenuCommandBuilder;
                    let description = 'Không có mô tả';
            
                    if ('description' in commandFile.data) { // Kiểm tra có thuộc tính 'description' không
                        description = commandFile.data.description || 'Không có mô tả';
                    }
                    if (isContextMenuCommand) description = commandFile.description || 'Không có mô tả';

                    // Lấy tên file mà không có phần mở rộng
                    const fileNameWithoutExt = path.basename(filePath, path.extname(filePath));
            
                    // Lấy tên thư mục chứa file nếu là subcommand
                    let commandName = commandFile.data.name;
                    if (isSubcommand = commandFile.data instanceof SlashCommandBuilder) {
                        // const parentFolder = path.basename(path.dirname(filePath)); // Lấy tên thư mục chứa file lệnh
                        commandName = `${commandName}`; // ${parentFolder}
                    } else if (isSubcommand = commandFile.data instanceof SlashCommandSubcommandBuilder) {
                        const parentFolder = path.basename(path.dirname(filePath)); // Lấy tên thư mục chứa file lệnh
                        commandName = `${parentFolder} ${commandName}`; // ${parentFolder}
                    }
