...
let verifyusersSchema = new Schema({
    ...
    { collection: 'Captcha mặc định đã hoàn thành' }
);
...
let captchaSchema = new Schema({
    ...
    { collection: 'Captcha mặc định chưa hoàn thành' }
);

// Tạo model từ các schema
const VerifyUsers = model("verifyusers", verifyusersSchema); // Model cho danh sách người dùng đã xác thực
const Captcha = model("Captcha", captchaSchema); // Model cho CAPTCHA
